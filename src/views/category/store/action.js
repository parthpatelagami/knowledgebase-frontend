  ...
  async (event, { getState, rejectWithValue }) => {
    try {
      ...
      const postData = {
        ...
        created_by: getState().auth.userData.user_id,
      };
      const response = await axios.post(
  ...
  async (event, { getState, rejectWithValue }) => {
    try {
      const postData = {
        ...
        updated_by: getState().auth.userData.user_id,
      };
...
export const deleteCategory = createAsyncThunk(
  ...
  async (event, { getState, rejectWithValue }) => {
    try {
      const value = event.category_id !== undefined ? event.category_id : event;
      const response = await axios.post(
...
export const checkCategoryMapping = createAsyncThunk(
  "category/check_category_mapping",
  async (event, { getState, rejectWithValue }) => {
    try {
      const value = event.category_id !== undefined ? event.category_id : event;
      const response = await axios.post(
        `${process.env.REACT_APP_API_ENDPOINT}/api/v1/category/check_category_mapping`,
        {
          id: value,
        }
      );
