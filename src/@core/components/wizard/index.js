...
const Wizard = forwardRef((props, ref) => {
  // ** Props
  const { type, className, contentClassName, headerClassName, steps, separator, options, instance, setCurrentIndex } = props
  ...
  useEffect(() => {
    ...
    ref.current.addEventListener('shown.bs-stepper', function (event) {
      ...
      if (setCurrentIndex) { setCurrentIndex(event.detail.indexStep) }
    })
